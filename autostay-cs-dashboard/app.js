...
1])
      .slice(0, 3);
    hmPeakEl.innerHTML = top3Hours.length
      ? top3Hours.map(([h, v], i) =>
          `<span class="peak-tag" style="${i===0?'font-weight:700;color:var(--teal)':''}">` +
          `${h}시 <strong>${v}</strong>건</span>`
        ).join('')
      : '<span style="color:var(--muted);font-size:11px">데이터 없음</span>';
  }
}
  ...
  charts.cat = new Chart(el.getContext('2d'), {
    ...
    data: {
      ...
      datasets: [{
        ...
        borderColor: '#faf9f7', borderWidth: 2, hoverOffset: 5
      }]
    },
    options: {
...
function renderChannel(d) {
  const { sources } = d;
  if (!sources) return;
  const labels = ['자사 웹/앱', '전화'];
  const values = [sources.native || 0, sources.phone || 0];
  if ((sources.other || 0) > 0) { labels.push('기타'); values.push(sources.other); }
  if (charts.ch) charts.ch.destroy();
  const el = document.getElementById('channelChart');
  if (!el) return;
  charts.ch = new Chart(el.getContext('2d'), {
    type: 'bar',
    data: {
      labels,
      datasets: [{ data: values, backgroundColor: ['#0f766e','#0369a1','#a8a29e'], borderRadius: 4, barThickness: 28 }]
    },
    options: {
      indexAxis: 'y', responsive: true, maintainAspectRatio: false,
...
function renderResolution(d) {
  ...
  const avgNote = document.getElementById('avgResNote');
  if (avgNote && d.summary?.avgResolutionMin != null) {
    const m = d.summary.avgResolutionMin;
    avgNote.textContent = `평균 ${m}분 (~${Math.round(m/60*10)/10}시간) · 비동기 채팅 특성상 고객 미응답 포함`;
  }
}
...
function renderManagers(d) {
  ...
  if (!tbody) return;
  if (!managers.length) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;color:var(--muted);padding:20px">담당자 데이터 없음</td></tr>';
    return;
  }
  tbody.innerHTML = managers.map((m, i) => {
    const touchPct = Math.min(m.touchScore || 0, 100);
    const isActive = m.count > 0;
    const badge = i === 0 && m.count > 0 ? '<span class="badge-top">★ 주담당</span>'
                : !isActive ? '<span class="badge-off">비활성</span>' : '—';
    const opColor = (m.operatorScore||0) > 30 ? 'var(--teal)' : (m.operatorScore||0) > 10 ? '#b45309' : 'var(--muted)';
    const touchColor = (m.touchScore||0) > 50 ? 'var(--teal)' : (m.touchScore||0) > 20 ? '#b45309' : 'var(--muted)';
    const pctShare = Math.round(m.count / d.summary.totalChats * 100);
    return `
      <tr>
        ...
        <td>
          <div class="score-cell">
            ...
            ${m.count > 0 ? `<div class="score-bar"><div class="score-fill" style="width:${pctShare}%"></div></div>
            <span style="font-size:10.5px;color:var(--muted)">${pctShare}%</span>` : ''}
          </div>
        </td>
        <td class="num-r"><span style="font-weight:700;color:${opColor}">${m.operatorScore||0}</span></td>
        <td class="num-r">
          <div class="score-cell" style="justify-content:flex-end">
            <span style="font-weight:700;color:${touchColor}">${m.touchScore||0}</span>
            <div class="score-bar"><div class="score-fill" style="width:${touchPct}%;background:${touchColor}"></div></div>
          </div>
        </td>
        <td class="num-r" style="color:var(--muted);font-size:11px">${m.count > 0 ? Math.round(d.summary.avgResolutionMin||0) + '분' : '—'}</td>
        <td>${badge}</td>
      </tr>`;
  }).join('');
}
...
function renderBots(d) {
  const bots = d.bots || [];
  const groupCount = d.groupCount || 0;
  ...
  if (botNamesEl) botNamesEl.textContent = bots.map(b => b.name).join(' · ') || '—';
...
function updateBanner(d) {
  const el = document.getElementById('updatedAt');
  if (el) el.textContent = new Date(d.updatedAt).toLocaleString('ko-KR');
  const chName = document.getElementById('channelName');
  const chId = document.getElementById('channelId');
  if (chName) chName.textContent = d.channel?.name || '오토스테이 CS';
  if (chId) chId.textContent = d.channel?.id || '177015';
}

/* ─── Fetch Data ─────────────────────────────────────────────────────────── */
async function fetchData() {
  const url = `/api/data?days=${currentDays}`;
...
async function render() {
  try {
    ...
    renderVOC(data);
    renderBots(data);
    renderPeakAnalysis(data.peakAnalysis, data.managers || []);
